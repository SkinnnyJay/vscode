...
export type SpawnClaudeProcess = (command: string, args: readonly string[]) => SpawnedClaudeProcess;
...
function defaultSpawnProcess(command: string, args: readonly string[]): SpawnedClaudeProcess {
...
export class ClaudeAdapter implements ProviderAdapter {
	...
	constructor(options: ClaudeAdapterOptions = {}) {
		this.binaryName = options.binaryName ?? 'claude';
	...
	async stream(
		...
		return await new Promise<ProviderResponse>((resolve, reject) => {
			...
			child.on('close', (exitCode) => {
				...
				const classified = classifyProviderError(stderr || stdout || `Claude exited with code ${normalizedExitCode}`);
