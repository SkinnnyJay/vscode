...
- **Summary-contract regression check:** run `./scripts/test-verify-gates-summary.sh` (executed in Pointer quality and nightly verify workflows before sweeps).
  ...
  - Includes derived-counter fallback checks proving renderer computes pass/fail/skip/not-run/executed counts and `statusCounts` from `gates[]` and gate-id list fields when top-level counters are omitted, plus failed-exit-code derivation from `failedGateIds` + `gateExitCodeById`.
