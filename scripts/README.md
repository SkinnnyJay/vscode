...
- **Summary-contract regression check:** run `./scripts/test-verify-gates-summary.sh` (executed in Pointer quality and nightly verify workflows before sweeps).
  ...
  - Includes selected-gate status-map scope checks proving summary-provided status and per-gate maps are filtered to explicit `selectedGateIds` before fallback count/list derivation.
