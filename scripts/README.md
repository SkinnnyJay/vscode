...
- **Machine-readable summary:** each run also writes `<mode>-<timestamp>.json`; override with `--summary-json <path>` or `VSCODE_VERIFY_SUMMARY_FILE`.
  ...
  - Summary payload includes `schemaVersion`, `runId`, `invocation`, `startedAt`, `completedAt`, `totalDurationSeconds`, `continueOnFailure`, `dryRun`, `gateCount`, `passedGateCount`, `failedGateCount`, `skippedGateCount`, `notRunGateCount`, `executedGateCount`, `passRatePercent`, `executedDurationSeconds`, `averageExecutedDurationSeconds`, `slowestExecutedGateId`, `slowestExecutedGateDurationSeconds`, `failedGateId`, `failedGateExitCode`, `failedGateIds`, `failedGateExitCodes`, `notRunGateIds`, plus per-gate `status`, `attempts`, `durationSeconds`, `exitCode`, `startedAt`, and `completedAt` (`null` when a gate was not run).
- **GitHub step summary helper:** use `./scripts/publish-verify-gates-summary.sh` to render the JSON payload into markdown for CI run summaries.
  ...
  - Rendered per-gate table includes command exit code for faster root-cause triage.
