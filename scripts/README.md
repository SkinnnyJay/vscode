...
- **Summary-contract regression check:** run `./scripts/test-verify-gates-summary.sh` (executed in Pointer quality and nightly verify workflows before sweeps).
  ...
  - Includes derived-counter fallback checks proving renderer computes pass/fail/skip/not-run/executed counts, retry/pass rates, retry backoff totals/share, duration aggregates, fastest/slowest executed gates, and `statusCounts` from `gates[]` and gate-id list fields when top-level scalar metrics are omitted, plus failed-exit-code derivation from `failedGateIds` + `gateExitCodeById`.
