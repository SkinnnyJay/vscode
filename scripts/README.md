...
- **Machine-readable summary:** each run also writes `<mode>-<timestamp>.json`; override with `--summary-json <path>` or `VSCODE_VERIFY_SUMMARY_FILE`.
  - Current summary schema version: `5`.
  - Summary payload includes `schemaVersion`, `runId`, `exitReason`, `invocation`, `startedAt`, `completedAt`, `totalDurationSeconds`, `continueOnFailure`, `dryRun`, `gateCount`, `passedGateCount`, `failedGateCount`, `skippedGateCount`, `notRunGateCount`, `executedGateCount`, `totalRetryCount`, `totalRetryBackoffSeconds`, `retriedGateCount`, `retriedGateIds`, `passRatePercent`, `executedDurationSeconds`, `averageExecutedDurationSeconds`, `slowestExecutedGateId`, `slowestExecutedGateDurationSeconds`, `fastestExecutedGateId`, `fastestExecutedGateDurationSeconds`, `failedGateId`, `failedGateExitCode`, `failedGateIds`, `failedGateExitCodes`, `notRunGateIds`, plus per-gate `status`, `attempts`, `retryCount`, `retryBackoffSeconds`, `durationSeconds`, `exitCode`, `startedAt`, `completedAt`, and `notRunReason` (`null` when not applicable).
- **GitHub step summary helper:** use `./scripts/publish-verify-gates-summary.sh` to render the JSON payload into markdown for CI run summaries.
  ...
  - Failed/not-run/retry metadata includes exit reason, first-failure pointers, complete failed/not-run/retried gate lists, and per-gate not-run reasons.
