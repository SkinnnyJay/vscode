...
- **Summary-contract regression check:** run `./scripts/test-verify-gates-summary.sh` (executed in Pointer quality and nightly verify workflows before sweeps).
  ...
  - Includes selected-gate status-map scope checks proving summary-provided status/per-gate maps, scalar failure metadata (`failedGateId`, `failedGateExitCode`, `blockedByGateId`), scalar slow/fast metadata (`slowestExecutedGateId`, `fastestExecutedGateId` + durations), start/end timestamps (`startedAt`, `completedAt`), failed exit-code list alignment (`failedGateIds` + `failedGateExitCodes`, with ambiguous exit-code lists ignored when IDs are absent), conflicting scalar/status counters (`gateCount`, `passed/failed/skipped/not-run/executed`, `statusCounts`), and conflicting aggregate metrics (`retried/total-retry`, duration totals/averages, retry/pass/share rates, `totalDurationSeconds`) are filtered/ignored to preserve explicit `selectedGateIds` scope.
