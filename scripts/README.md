...
- **Summary-contract regression check:** run `./scripts/test-verify-gates-summary.sh` (executed in Pointer quality and nightly verify workflows before sweeps).
  ...
  - Includes selected-gate status-map scope checks proving summary-provided status/per-gate maps and scalar failure metadata (`failedGateId`, `failedGateExitCode`, `blockedByGateId`) are filtered to explicit `selectedGateIds` before fallback derivation.
