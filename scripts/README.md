...
- **Summary-contract regression check:** run `./scripts/test-verify-gates-summary.sh` (executed in Pointer quality and nightly verify workflows before sweeps).
  ...
  - Includes derived-counter fallback checks proving renderer computes pass/fail/skip/not-run/executed counts and `statusCounts` directly from `gates[]` when top-level counters are omitted.
