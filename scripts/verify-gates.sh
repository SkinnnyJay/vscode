...
write_gate_status_by_id_json() {
	...
	for i in "${!gate_ids[@]}"; do
		...
		echo "    \"$(json_escape "${gate_ids[$i]}")\": \"$(json_escape "${gate_results[$i]}")\"${delimiter}"
	done
}
...
write_gate_retry_count_by_id_json() {
	...
	for i in "${!gate_ids[@]}"; do
		...
		local retry_count
		retry_count="$(compute_gate_retry_count_by_index "$i")"
		echo "    \"$(json_escape "${gate_ids[$i]}")\": ${retry_count}${delimiter}"
	done
}
...
write_gate_not_run_reason_by_id_json() {
	local i
	for i in "${!gate_ids[@]}"; do
		local delimiter=","
		if ((i == ${#gate_ids[@]} - 1)); then
		...
		if [[ -n "${gate_not_run_reasons[$i]}" ]]; then
			echo "    \"$(json_escape "${gate_ids[$i]}")\": \"$(json_escape "${gate_not_run_reasons[$i]}")\"${delimiter}"
		else
			echo "    \"$(json_escape "${gate_ids[$i]}")\": null${delimiter}"
		fi
	done
}

write_gate_attempt_count_by_id_json() {
	local i
	for i in "${!gate_ids[@]}"; do
		local delimiter=","
		if ((i == ${#gate_ids[@]} - 1)); then
		...
		echo "    \"$(json_escape "${gate_ids[$i]}")\": ${gate_attempt_counts[$i]}${delimiter}"
	done
}
...
write_summary_json() {
	...
	mkdir -p "$(dirname "$SUMMARY_FILE")"
	{
		...
		write_gate_duration_seconds_by_id_json
		echo "  },"
		echo "  \"gateNotRunReasonById\": {"
		write_gate_not_run_reason_by_id_json
		echo "  },"
		echo "  \"gateAttemptCountById\": {"
		write_gate_attempt_count_by_id_json
		echo "  },"
