...
- **Selected scope filtering for summary-provided maps/lists (2026-02-15 PM)** Closed explicit-map selection leak:
  ...
  - Explicit empty selected `attentionGateIds` is now explicitly contract-validated as authoritative even when selected retried gates exist.
  - Explicit empty selected `nonSuccessGateIds` is now explicitly contract-validated to keep selected retried-gate attention fallback behavior intact.
  ...
  - `scripts/test-verify-gates-summary.sh` now adds `selected_run_state_not_run_blocked_selected_continued_conflict_scope` scenario and verifies conflicting explicit `completed-with-failures` / `failed-continued` metadata is ignored under selected row-derived blocked-reason fail-fast evidence.
  ...
  - `scripts/test-verify-gates-summary.sh` now adds `selected_run_state_not_run_blocked_selected_dry_reason_scope` scenario and verifies conflicting explicit dry-run reason/classification metadata is also ignored under selected row-derived blocked-reason fail-fast evidence.
  ...
  - `scripts/test-verify-gates-summary.sh` now adds `selected_explicit_retried_nonselected_scope` scenario and verifies explicit retried IDs fully scoped out by `selectedGateIds` collapse to `Retried gates: none`, zero retry-count maps, and zero retry aggregates.
  ...
  - `scripts/test-verify-gates-summary.sh` now adds `scalar_none_sentinel_gate_ids_case_scope` scenario and verifies case/whitespace `none` scalar sentinels are suppressed under selected scope too.
...
- **Explicit empty retried-list aggregate precedence (2026-02-15 PM)** Hardened retry aggregate derivation:
  - `scripts/publish-verify-gates-summary.sh` now derives fallback retry aggregates (`totalRetryCount`, `totalRetryBackoffSeconds`) from scoped `retriedGateIds` rather than all retry-count map entries.
  - This keeps explicit retried-list overrides (`retriedGateIds: []`) authoritative when sparse producer payloads also include stale `gateRetryCountById` values.
  - `scripts/test-verify-gates-summary.sh` now adds `selected_explicit_empty_retried_with_retry_map_scope` and verifies explicit empty retried lists preserve:
    - `Retried gates: none`
    - `Retried gate count: 0`
    - `Total retries: 0`
...
- **Explicit retried-gate minimum retry-count normalization (2026-02-16 AM)** Hardened sparse retry-map consistency:
  ...
  - When explicit retried IDs are fully scoped out by `selectedGateIds`, retried metadata now consistently collapses to empty/zero values.
  ...
  - `scripts/test-verify-gates-summary.sh` now adds `explicit_retried_subset_retry_map` and `explicit_retried_missing_retry_map_key` and verifies unscoped explicit retried subsets still constrain aggregate retries/backoff and attention lists (with non-retried map entries zeroed) when retry-count maps include additional gate IDs or omit explicit retried IDs.
...
- **Selected aggregate scalar no-evidence fallback coverage (2026-02-16 AM)** Completed selected-scope sparse aggregate matrix:
  - `scripts/test-verify-gates-summary.sh` now adds `selected_aggregate_metrics_no_evidence_scope`.
  - Scenario verifies selected scope with conflicting explicit aggregate retry/duration/rate scalars but no selected retry/duration execution evidence.
  - Assertions confirm selected aggregate scalar inputs are ignored under explicit selected scope and normalized to deterministic no-evidence outputs:
    - retry totals/counts/backoff -> `0`
    - executed duration total -> `0s`
    - duration average + retry/pass/share rates -> `n/a`.
...
- **Selected decimal-string aggregate scalar scope coverage (2026-02-16 AM)** Extended selected aggregate normalization matrix:
  - `scripts/test-verify-gates-summary.sh` now adds `selected_aggregate_metrics_decimal_string_scope`.
  - Scenario injects decimal numeric-string aggregate retry/duration/rate scalars under explicit selected scope with valid selected retry/duration evidence.
...
- **Selected mixed boundary/overflow rate suppression coverage (2026-02-16 AM)** Extended selected sparse/evidence matrices:
  - `scripts/test-verify-gates-summary.sh` now adds:
    - `selected_aggregate_metrics_rate_scalar_mixed_boundary_scope`,
    - `selected_aggregate_metrics_rate_scalar_mixed_boundary_no_evidence_scope`.
  - Scope scenario mixes boundary + overflow selected rate scalars with selected execution evidence.
  - No-evidence scenario mixes boundary + overflow selected rate scalars without selected execution evidence.
...
- **Selected explicit empty partition-list + status-map coexistence coverage (2026-02-16 PM)** Extended selected partition precedence matrix:
  - `scripts/test-verify-gates-summary.sh` now adds `selected_explicit_empty_partition_lists_status_map_scope`.
  - Scenario injects explicit empty selected partition lists (`passed/failed/skipped/notRun` all `[]`) alongside selected status-map evidence (`lint=pass`, `typecheck=fail`).
...
- **Selected partial status-map+partition suppression of explicit-zero raw status-count bundles (2026-02-16 PM)** Extended selected mixed-evidence suppression matrix:
  - `scripts/test-verify-gates-summary.sh` now adds `selected_status_counts_partial_zero_raw_status_map_partition_scope`.
  - Scenario injects selected partial status-map evidence with partition fallback plus explicit zero raw `statusCounts` values and conflicting selected scalar counts.
...
- **Selected status-map pass precedence suppression of conflicting executed-count scalar under non-success partition conflict coverage (2026-02-16 PM)** Tightened selected executed/non-success precedence matrix:
  - `scripts/test-verify-gates-summary.sh` updates `selected_non_success_status_precedence_scope` to include conflicting `executedGateCount: 0`.
  - Scenario keeps selected status-map pass evidence for `lint` while selected not-run partition membership still conflicts for diagnostics.
  - Assertions confirm selected executed metadata/rates stay status-map-driven (`Executed gates: 1`, list `lint`, pass rate `100%`) while conflicting selected scalar executed-count zero remains ignored.
  - `scripts/README.md` selected executed precedence note updated to explicitly mention selected status-map pass precedence over conflicting selected not-run partition membership with scalar executed-count suppression.
  **Why:** closes the selected status-map-pass + conflicting not-run partition branch so conflicting scalar executed-count values cannot nullify selected status-map-driven executed metadata/rates.
- **Selected status-map fail precedence suppression of conflicting executed-count scalar under non-success partition conflict coverage (2026-02-16 PM)** Extended selected executed/non-success precedence matrix:
  - `scripts/test-verify-gates-summary.sh` now adds `selected_non_success_status_precedence_fail_scope`.
  - Scenario keeps selected status-map fail evidence for `lint` while selected not-run partition membership still conflicts for diagnostics and a conflicting `executedGateCount: 0` scalar is provided.
...
- **Selected status-map not-run precedence suppression of conflicting executed-count scalar under partition conflict coverage (2026-02-16 PM)** Extended selected executed/non-success precedence matrix:
  - `scripts/test-verify-gates-summary.sh` now adds `selected_non_success_status_precedence_not_run_scope`.
  - Scenario keeps selected status-map not-run evidence for `lint` while conflicting selected pass-partition membership and conflicting `executedGateCount: 4` scalar are provided.
  - Assertions confirm selected executed metadata stays non-executed/status-map-not-run-driven (`Executed gates: 0`, list `none`, pass rate `n/a`) while conflicting selected partition/scalar executed evidence remains ignored.
...
- **Unscoped numeric-string executed-count scalar override over sparse status-map/partition executed fallback coverage (2026-02-16 PM)** Extended unscoped executed scalar normalization matrix:
  - `scripts/test-verify-gates-summary.sh` now adds `unscoped_executed_string_scalar_count_overrides_partial_status_map`.
  - Scenario keeps sparse unscoped executed fallback evidence from partial status-map + partition lists while executed lists remain implicit and explicit numeric-string `executedGateCount: ' 5 '` is provided.
  - Assertions confirm normalized numeric-string scalar executed-count remains authoritative (`Executed gates: 5`) while sparse derived executed-list labels stay visible (`Executed gates list: typecheck, lint`).
...
- **Unscoped numeric-string executed-count scalar override over explicit empty executed-list coverage (2026-02-16 PM)** Extended unscoped executed scalar normalization matrix:
  - `scripts/test-verify-gates-summary.sh` now adds `unscoped_executed_string_scalar_count_overrides_empty_list`.
  - Scenario keeps explicit unscoped empty executed-list evidence while injecting explicit numeric-string `executedGateCount: ' 5 '` and sparse status/retry evidence.
  - Assertions confirm normalized numeric-string scalar executed-count remains authoritative (`Executed gates: 5`) while explicit empty executed-list label remains visible (`Executed gates list: none`).
...
- **Selected numeric-string zero executed-count scalar suppression under partial selected status-map fallback coverage (2026-02-16 PM)** Extended selected sparse executed fallback precedence matrix:
  - `scripts/test-verify-gates-summary.sh` now adds `selected_executed_string_zero_scalar_ignored_partial_status_map_scope`.
  - Scenario omits explicit `executedGateIds`, keeps selected sparse `gateStatusById` + selected partition fallback evidence, and injects conflicting numeric-string zero `executedGateCount: ' 0 '`.
  - Assertions confirm selected sparse status-map/partition fallback remains authoritative (`Executed gates: 2`, pass rate `50%`, executed list `lint, typecheck`) while conflicting numeric-string zero scalar is ignored.
...
- **Selected integer executed-count scalar suppression under partial selected status-map fallback coverage (2026-02-16 PM)** Extended selected sparse executed fallback precedence matrix:
  - `scripts/test-verify-gates-summary.sh` now adds `selected_executed_scalar_ignored_partial_status_map_scope`.
  - Scenario omits explicit `executedGateIds`, keeps selected sparse `gateStatusById` + selected partition fallback evidence, and injects conflicting integer `executedGateCount: 5`.
  - Assertions confirm selected sparse status-map/partition fallback remains authoritative (`Executed gates: 2`, pass rate `50%`, executed list `lint, typecheck`) while conflicting integer scalar is ignored.
  - Assertions also reject integer-scalar-driven fallback (`Executed gates: 5`, pass rate `20%`) and verify selected non-success/attention metadata plus partial selected status-map visibility remain aligned with sparse fallback evidence.
  - `scripts/README.md` selected executed fallback note updated to explicitly mention integer non-zero scalar suppression alongside numeric-string scalar suppression for selected sparse fallback paths.
  **Why:** closes the selected integer non-zero executed-count scalar + partial selected status-map/partition fallback branch so integer non-zero scalars cannot suppress selected fallback-derived executed metadata/rates when explicit executed lists are absent.
- **Selected raw statusCounts executed-branch suppression under partial selected status-map fallback coverage (2026-02-16 PM)** Extended selected sparse executed fallback precedence matrix:
  - `scripts/test-verify-gates-summary.sh` now adds `selected_executed_raw_status_counts_ignored_partial_status_map_scope`.
  - Scenario omits explicit `executedGateIds`, keeps selected sparse `gateStatusById` + selected partition fallback evidence, and injects conflicting raw `statusCounts` (`pass: 0`, `fail: 0`, `skip: 5`, `not-run: 0`).
  - Assertions confirm selected sparse status-map/partition fallback remains authoritative (`Executed gates: 2`, pass rate `50%`, executed list `lint, typecheck`) while conflicting raw `statusCounts` executed branch evidence is ignored.
...
- **Selected numeric-string raw statusCounts executed-branch suppression under partial selected status-map fallback coverage (2026-02-16 PM)** Extended selected sparse executed fallback precedence matrix:
  - `scripts/test-verify-gates-summary.sh` now adds `selected_executed_raw_status_counts_string_ignored_partial_status_map_scope`.
  - Scenario omits explicit `executedGateIds`, keeps selected sparse `gateStatusById` + selected partition fallback evidence, and injects conflicting numeric-string raw `statusCounts` (`pass: '2'`, `fail: '3'`, `skip: '0'`, `not-run: '0'`).
  - Assertions confirm selected sparse status-map/partition fallback remains authoritative (`Executed gates: 2`, pass rate `50%`, executed list `lint, typecheck`) while conflicting numeric-string raw `statusCounts` executed branch evidence is ignored.
  - Assertions also reject numeric-string-raw-count-driven fallback (`Executed gates: 5`, pass rate `40%`) and verify selected non-success/attention metadata plus partial selected status-map visibility remain aligned with sparse fallback evidence.
  - `scripts/README.md` selected executed fallback note updated to explicitly mention numeric-string raw `statusCounts` executed-branch suppression alongside integer raw status-count suppression.
  **Why:** closes the selected numeric-string raw `statusCounts` executed-branch + partial selected status-map/partition fallback branch so numeric-string raw count objects cannot suppress selected fallback-derived executed metadata/rates when explicit executed lists are absent.
- **Selected partially malformed raw statusCounts executed-branch suppression under partial selected status-map fallback coverage (2026-02-16 PM)** Extended selected sparse executed fallback precedence matrix:
  - `scripts/test-verify-gates-summary.sh` now adds `selected_executed_raw_status_counts_partial_malformed_ignored_partial_status_map_scope`.
  - Scenario omits explicit `executedGateIds`, keeps selected sparse `gateStatusById` + selected partition fallback evidence, and injects conflicting partially malformed raw `statusCounts` (`pass: '4'`, `fail: 'bad'`, `skip: '0'`, `not-run: null`).
  - Assertions confirm selected sparse status-map/partition fallback remains authoritative (`Executed gates: 2`, pass rate `50%`, executed list `lint, typecheck`) while conflicting partially malformed raw `statusCounts` executed branch evidence is ignored.
  - Assertions also reject partially-malformed-raw-count-driven fallback (`Executed gates: 4`, pass rate `100%`) and verify selected non-success/attention metadata plus partial selected status-map visibility remain aligned with sparse fallback evidence.
  - `scripts/README.md` selected executed fallback note updated to explicitly mention partially malformed raw `statusCounts` suppression alongside integer/numeric-string raw status-count suppression.
  **Why:** closes the selected partially malformed raw `statusCounts` executed-branch + partial selected status-map/partition fallback branch so partially malformed raw count bundles cannot suppress selected fallback-derived executed metadata/rates when explicit executed lists are absent.
- **Selected scalar+raw statusCounts combined executed-branch suppression under partial selected status-map fallback coverage (2026-02-16 PM)** Extended selected sparse executed fallback precedence matrix:
  - `scripts/test-verify-gates-summary.sh` now adds `selected_executed_scalar_and_raw_status_counts_ignored_partial_status_map_scope`.
  - Scenario omits explicit `executedGateIds`, keeps selected sparse `gateStatusById` + selected partition fallback evidence, and injects conflicting scalar `executedGateCount: 5` plus conflicting raw `statusCounts` (`pass: 2`, `fail: 3`, `skip: 0`, `not-run: 0`).
  - Assertions confirm selected sparse status-map/partition fallback remains authoritative (`Executed gates: 2`, pass rate `50%`, executed list `lint, typecheck`) while both conflicting scalar and raw `statusCounts` executed branch evidence are ignored.
  - Assertions also reject combined-conflict fallback (`Executed gates: 5`, pass rate `40%`) and verify selected non-success/attention metadata plus partial selected status-map visibility remain aligned with sparse fallback evidence.
  - `scripts/README.md` selected executed fallback note updated to explicitly mention combined selected scalar-plus-raw status-count conflict suppression.
  **Why:** closes the selected combined scalar + raw `statusCounts` executed-branch conflict path so dual conflicting aggregate sources cannot suppress selected fallback-derived executed metadata/rates when explicit executed lists are absent.
- **Selected numeric-string scalar+raw statusCounts combined executed-branch suppression under partial selected status-map fallback coverage (2026-02-16 PM)** Extended selected sparse executed fallback precedence matrix:
  - `scripts/test-verify-gates-summary.sh` now adds `selected_executed_string_scalar_and_raw_status_counts_ignored_partial_status_map_scope`.
  - Scenario omits explicit `executedGateIds`, keeps selected sparse `gateStatusById` + selected partition fallback evidence, and injects conflicting numeric-string scalar `executedGateCount: ' 5 '` plus conflicting raw `statusCounts` (`pass: 2`, `fail: 3`, `skip: 0`, `not-run: 0`).
  - Assertions confirm selected sparse status-map/partition fallback remains authoritative (`Executed gates: 2`, pass rate `50%`, executed list `lint, typecheck`) while both conflicting numeric-string scalar and raw `statusCounts` executed branch evidence are ignored.
  - Assertions also reject combined-conflict fallback (`Executed gates: 5`, pass rate `40%`) and verify selected non-success/attention metadata plus partial selected status-map visibility remain aligned with sparse fallback evidence.
  - `scripts/README.md` selected executed fallback note updated to explicitly mention combined selected numeric-string scalar-plus-raw status-count conflict suppression alongside integer scalar-plus-raw suppression.
  **Why:** closes the selected combined numeric-string scalar + raw `statusCounts` executed-branch conflict path so normalized numeric-string scalar plus raw aggregate conflicts cannot suppress selected fallback-derived executed metadata/rates when explicit executed lists are absent.
- **Selected numeric-string zero scalar+raw statusCounts combined executed-branch suppression under partial selected status-map fallback coverage (2026-02-16 PM)** Extended selected sparse executed fallback precedence matrix:
  - `scripts/test-verify-gates-summary.sh` now adds `selected_executed_string_zero_scalar_and_raw_status_counts_ignored_partial_status_map_scope`.
  - Scenario omits explicit `executedGateIds`, keeps selected sparse `gateStatusById` + selected partition fallback evidence, and injects conflicting numeric-string zero scalar `executedGateCount: ' 0 '` plus conflicting raw `statusCounts` (`pass: 2`, `fail: 3`, `skip: 0`, `not-run: 0`).
  - Assertions confirm selected sparse status-map/partition fallback remains authoritative (`Executed gates: 2`, pass rate `50%`, executed list `lint, typecheck`) while both conflicting numeric-string zero scalar and raw `statusCounts` executed branch evidence are ignored.
...
- **Unscoped integer scalar+raw statusCounts combined executed precedence parity coverage (2026-02-16 PM)** Extended unscoped sparse executed precedence matrix:
  - `scripts/test-verify-gates-summary.sh` now adds:
    - `unscoped_executed_scalar_and_raw_status_counts_overrides_partial_status_map`
    - `unscoped_executed_zero_scalar_and_raw_status_counts_overrides_partial_status_map`
  - Both scenarios omit explicit `executedGateIds`, keep sparse unscoped `gateStatusById` + `failedGateIds` executed-list fallback evidence, and provide raw `statusCounts` (`pass: 2`, `fail: 3`, `skip: 0`, `not-run: 0`) plus conflicting integer `executedGateCount` scalars (`5` and `0`).
  - Assertions confirm unscoped raw `statusCounts` remains authoritative for pass/fail counters while executed count/rate metrics follow integer scalar precedence (`Executed gates: 5`, pass/retry rates `40%`/`20%`; and `Executed gates: 0` with executed-rate `n/a` for zero-scalar form), with sparse executed-list labels still preserved (`typecheck, lint`).
  - Assertions reject implicit sparse executed fallback branches (`Executed gates: 2`, `Pass rate: 50%`, `Retry rate: 50%`) and reject non-zero executed/rate leakage in the zero-scalar path.
  - `scripts/README.md` unscoped aggregate precedence notes now explicitly document combined integer and numeric-string executed scalar + raw `statusCounts` precedence, including both integer and normalized numeric-string zero-scalar behavior.
  - Validation:
    - `./scripts/test-verify-gates-summary.sh` ✅
    - `make lint` ✅
    - `./scripts/verify-gates.sh --quick` ✅ (`quick-20260216T225111Z`, classification `success-with-retries`)
...
- **Unscoped explicit non-empty executed-list numeric-string scalar+raw statusCounts precedence parity coverage (2026-02-16 PM)** Extended unscoped executed override matrix:
  - `scripts/test-verify-gates-summary.sh` now adds:
    - `unscoped_executed_string_scalar_and_raw_status_counts_overrides_explicit_list`
    - `unscoped_executed_string_zero_scalar_and_raw_status_counts_overrides_explicit_list`
  - Both scenarios keep explicit non-empty `executedGateIds: ['lint', 'typecheck']`, retain sparse pass/fail partition and retry evidence, and inject raw `statusCounts` (`pass: 2`, `fail: 3`, `skip: 0`, `not-run: 0`) with numeric-string `executedGateCount` scalars (`' 5 '`, `' 0 '`).
  - Assertions confirm deterministic split precedence:
    - raw `statusCounts` remains authoritative for pass/fail count lines and rendered status-count metadata
    - normalized numeric-string executed-count scalar remains authoritative for denominator/rate semantics (`Executed gates: 5` + pass/retry `40%`/`20%`; or `Executed gates: 0` + executed rates `n/a`)
    - explicit non-empty executed-list label (`Executed gates list: lint, typecheck`) remains intact while scalar denominator controls rates
    - sparse fallback branches are suppressed (`Executed gates: 2` / `Pass rate: 50%`) and non-zero leakage is blocked in the normalized zero-scalar path.
  - `scripts/README.md` unscoped aggregate precedence notes now explicitly include combined integer/numeric-string scalar+raw precedence for explicit non-empty executed-list overrides.
  - Validation:
    - `./scripts/test-verify-gates-summary.sh` ✅
    - `make lint` ✅
...
- **Selected explicit empty executed-list partial malformed raw statusCounts scalar+raw suppression coverage (2026-02-17 AM)** Extended selected-scope executed override matrix:
  - `scripts/test-verify-gates-summary.sh` now adds:
    - `selected_executed_scalar_partial_malformed_raw_status_counts_ignored_empty_list_scope`
    - `selected_executed_string_scalar_partial_malformed_raw_status_counts_ignored_empty_list_scope`
    - `selected_executed_string_zero_scalar_partial_malformed_raw_status_counts_ignored_empty_list_scope`
  - Scenarios retain selected explicit empty executed evidence (`selectedGateIds: ['lint', 'typecheck']`, `executedGateIds: []`) and selected partition+retry metadata while injecting conflicting mixed-source aggregates with partial malformed raw `statusCounts` (`pass: 'bad'`, `fail: 3`, `skip: null`, `not-run: 'bad'`) plus integer, numeric-string, and numeric-string-zero `executedGateCount` scalars.
  - Assertions confirm selected explicit-empty-list precedence remains deterministic:
    - selected counters/status-count metadata stay selected-list scoped (`Passed/Failed/Executed = 1/1/0`, `Status counts: {"pass":1,"fail":1,"skip":0,"not-run":0}`)
    - explicit empty executed-list/rate metadata stays authoritative (`Executed gates list: none`, executed pass/retry rates `n/a`)
    - selected attention metadata remains stable (`lint, typecheck`)
    - conflicting malformed/raw+scalar branches are suppressed (`Failed gates: 3`, `Executed gates: 5`, `Pass rate: 20%` do not appear).
  - `scripts/README.md` selected explicit executed-list override notes now explicitly include partial malformed raw scalar+raw suppression under explicit selected empty executed-list evidence.
  - Validation:
    - `./scripts/test-verify-gates-summary.sh` ✅
    - `make lint` ✅
    - `./scripts/verify-gates.sh --quick` ✅ (`quick-20260217T010124Z`, classification `success-with-retries`)
...
- **Selected non-selected map-only valid-zero raw statusCounts scoping parity (2026-02-17 AM)** Extended selected-scope status-count matrix:
  - `scripts/test-verify-gates-summary.sh` now adds:
    - `selected_status_counts_zero_raw_nonselected_map_only_scope`
    - `selected_status_counts_zero_raw_nonselected_map_only_string_zero_scope`
  - Scenarios preserve selected scope (`selectedGateIds: ['lint', 'typecheck']`) while providing non-selected map-only evidence (`gateStatusById`/`gateDurationSecondsById` for `build`/`deploy`) and valid-zero raw `statusCounts` projections, with conflicting scalar counters and integer/numeric-string executed-denominator variants.
  - Assertions confirm selected map-only valid-zero isolation:
    - selected metadata remains canonical (`Selected gates`, `Gate count: 2`)
    - selected counters/status counts remain collapsed (`Passed/Failed/Skipped/Not-run = 0`, `Status counts: {"pass":0,"fail":0,"skip":0,"not-run":0}`)
    - executed metadata remains empty and rates stay `n/a` (`Executed gates: 0`, list `none`, pass/retry rates `n/a`)
    - non-selected map-only evidence and scalar/raw leak branches remain suppressed (`Failed gates: 1`, `Executed gates: 2`, `Executed gates: 4` absent).
  - `scripts/README.md` selected aggregate precedence notes now explicitly include map-only valid-zero raw `statusCounts` variants in selected non-selected-evidence scoping behavior.
  - Validation:
    - `./scripts/test-verify-gates-summary.sh` ✅
    - `make lint` ✅
    - `./scripts/verify-gates.sh --quick` ✅
...
- **Selected/non-selected mixed scoped-evidence explicit zero-denominator parity under valid-zero raw statusCounts (2026-02-17 AM)** Extended selected-scope status-count matrix:
  - `scripts/test-verify-gates-summary.sh` now adds:
    - `selected_status_counts_zero_raw_mixed_selected_nonselected_zero_scope`
    - `selected_status_counts_zero_raw_mixed_selected_nonselected_string_zero_scope`
  - Scenarios extend mixed selected+non-selected explicit-list valid-zero raw branches with explicit selected zero denominators (`executedGateCount: 0` and `' 0 '`) while preserving selected and non-selected memberships in executed/pass/fail lists.
  - Assertions confirm mixed explicit-zero denominator parity:
    - selected metadata remains canonical (`Selected gates`, `Gate count: 2`)
    - selected counters/status counts preserve scoped selected evidence (`Passed/Failed/Skipped/Not-run = 1/1/0/0`, `Status counts: {"pass":1,"fail":1,"skip":0,"not-run":0}`)
    - executed metadata/rates remain selected-scoped (`Executed gates: 2`, list `lint, typecheck`, pass rate `50%`) despite conflicting zero-denominator scalars
    - explicit-zero scalar/zero-raw and non-selected list leakage is suppressed (`Passed gates: 0`, `Executed gates: 0`, `Executed gates list: lint, typecheck, build` absent).
  - `scripts/README.md` selected aggregate precedence notes now explicitly include mixed selected+non-selected explicit-list valid-zero raw branches with integer/numeric-string zero-denominator scalar variants.
  - Validation:
    - `./scripts/test-verify-gates-summary.sh` ✅
    - `make lint` ✅
    - `./scripts/verify-gates.sh --quick` ✅
...
- **Selected/non-selected mixed partial-malformed map-only parity (including explicit zero denominators) (2026-02-17 AM)** Extended selected-scope status-count matrix:
  - `scripts/test-verify-gates-summary.sh` now adds:
    - `selected_status_counts_partial_malformed_mixed_selected_nonselected_map_only_scope`
    - `selected_status_counts_partial_malformed_mixed_selected_nonselected_map_only_string_scope`
    - `selected_status_counts_partial_malformed_mixed_selected_nonselected_map_only_zero_scope`
    - `selected_status_counts_partial_malformed_mixed_selected_nonselected_map_only_string_zero_scope`
  - Scenarios provide mixed selected+non-selected status-map evidence (`gateStatusById` includes selected and extra non-selected IDs) with partially malformed raw `statusCounts` and conflicting scalar counters, while omitting explicit executed/passed/failed lists to force map-only fallback behavior across integer/numeric-string non-zero and zero selected executed-denominator scalar variants.
  - Assertions confirm mixed partial-malformed map-only parity:
    - selected metadata remains canonical (`Selected gates`, `Gate count: 2`)
    - selected counters/status counts preserve scoped selected map evidence (`Passed/Failed/Skipped/Not-run = 1/1/0/0`, `Status counts: {"pass":1,"fail":1,"skip":0,"not-run":0}`)
    - executed metadata/rates remain selected-scoped (`Executed gates: 2`, list `lint, typecheck`, pass rate `50%`) despite conflicting malformed/raw and explicit zero-denominator scalar inputs
    - partial-malformed scalar/raw and non-selected map leakage is suppressed (`Passed gates: 0`, `Executed gates: 0`, `Executed gates list: lint, typecheck, build` absent).
  - `scripts/README.md` selected aggregate precedence notes now explicitly include mixed selected+non-selected partial-malformed raw parity for both explicit-list-backed and map-only evidence paths.
  - Validation:
    - `./scripts/test-verify-gates-summary.sh` ✅
    - `make lint` ✅
    - `./scripts/verify-gates.sh --quick` ✅
...
- **Selected/non-selected mixed valid-zero raw partition-only parity (including explicit zero denominators) (2026-02-17 AM)** Extended selected-scope status-count matrix:
  - `scripts/test-verify-gates-summary.sh` now adds:
    - `selected_status_counts_zero_raw_mixed_selected_nonselected_partition_only_scope`
    - `selected_status_counts_zero_raw_mixed_selected_nonselected_partition_only_string_scope`
    - `selected_status_counts_zero_raw_mixed_selected_nonselected_partition_only_zero_scope`
    - `selected_status_counts_zero_raw_mixed_selected_nonselected_partition_only_string_zero_scope`
  - Scenarios provide mixed selected+non-selected partition-list evidence (`passedGateIds`/`failedGateIds` include selected and non-selected IDs) with valid-zero raw `statusCounts` and conflicting scalar counters, while omitting explicit executed lists and status maps to force partition-only executed fallback behavior across integer/numeric-string non-zero and zero selected executed-denominator scalar variants.
  - Assertions confirm mixed valid-zero raw partition-only parity:
    - selected metadata remains canonical (`Selected gates`, `Gate count: 2`)
    - selected counters/status counts preserve scoped selected partition evidence (`Passed/Failed/Skipped/Not-run = 1/1/0/0`, `Status counts: {"pass":1,"fail":1,"skip":0,"not-run":0}`)
    - executed metadata/rates remain selected-scoped (`Executed gates: 2`, pass rate `50%`) despite conflicting zero-raw and explicit zero-denominator scalar inputs
    - scalar/zero-raw and non-selected partition leakage is suppressed (`Passed gates: 0`, `Executed gates: 0`, `Executed gates list: lint, typecheck, build` absent).
  - `scripts/README.md` selected aggregate precedence notes now explicitly include valid-zero raw parity for explicit-list-backed, partition-only, and map-only evidence paths.
  - Validation:
    - `./scripts/test-verify-gates-summary.sh` ✅
    - `make lint` ✅
    - `./scripts/verify-gates.sh --quick` ✅
...
- **Selected/non-selected mixed non-executed partition-only parity (2026-02-17 AM)** Extended selected-scope status-count matrix:
  - `scripts/test-verify-gates-summary.sh` now adds:
    - `selected_status_counts_partial_malformed_mixed_selected_nonselected_nonexecuted_partition_only_scope`
  ...
  - Scenarios provide mixed selected+non-selected partition evidence where selected gates are skip/not-run only (non-executed) and non-selected gates carry pass/fail evidence, while omitting explicit executed/status-map lists to force partition-only fallback behavior under both partial malformed and valid-zero raw `statusCounts` bundles with conflicting scalar counters.
  - Assertions confirm mixed non-executed partition-only parity:
    - selected metadata remains canonical (`Selected gates`, `Gate count: 2`)
    - selected counters/status counts preserve scoped selected non-executed partition projections (`Passed/Failed/Skipped/Not-run = 0/0/1/1`, `Status counts: {"pass":0,"fail":0,"skip":1,"not-run":1}`)
    - executed metadata/rates remain selected-scoped no-execution (`Executed gates: 0`, list `none`, pass/retry rates `n/a`) across integer/numeric-string non-zero and zero selected executed-denominator scalar variants
    - non-selected executed partition leakage and scalar/raw override leakage is suppressed (`Passed/Failed = 1/1`, `Executed gates: 2/4`, `Failed gates: 3`, executed list `build, deploy` absent).
  - `scripts/README.md` selected aggregate precedence notes now explicitly call out partition-only selected non-executed projections for both valid-zero and partial-malformed mixed selected/non-selected branches.
  - Validation:
    - `./scripts/test-verify-gates-summary.sh` ✅
    - `make lint` ✅
    - `./scripts/verify-gates.sh --quick` ✅
  **Why:** closes mixed non-executed partition-only parity so selected skip/not-run partition evidence remains authoritative while non-selected executed partition evidence and conflicting scalar/raw bundles stay filtered.
- **Selected non-executed partition-only retry/duration scoping parity (2026-02-17 AM)** Hardened selected non-executed partition-only aggregate checks:
  - `scripts/test-verify-gates-summary.sh` now enriches:
    - `selected_status_counts_partial_malformed_mixed_selected_nonselected_nonexecuted_partition_only_scope`
    - `selected_status_counts_zero_raw_mixed_selected_nonselected_nonexecuted_partition_only_scope`
  - Added non-selected-only retry/duration evidence (`retriedGateIds`, `gateRetryCountById`, `totalRetryBackoffSeconds`, `gateDurationSecondsById`, `executedDurationSeconds`, `retryBackoffSharePercent`) to ensure selected non-executed partition scope does not leak non-selected aggregates.
  ...
  - `scripts/README.md` selected aggregate precedence notes now call out partition-only non-selected retry/duration aggregate suppression under selected non-executed scope.
  - Validation:
    - `./scripts/test-verify-gates-summary.sh` ✅
    - `make lint` ✅
    - `./scripts/verify-gates.sh --quick` ✅
...
- **Selected non-executed explicit-list zero-denominator retry/duration parity (2026-02-17 AM)** Extended selected non-executed explicit-list aggregate checks to explicit zero-denominator branches:
  - `scripts/test-verify-gates-summary.sh` now enriches:
    - `selected_status_counts_partial_malformed_mixed_selected_nonselected_nonexecuted_zero_scope`
    - `selected_status_counts_partial_malformed_mixed_selected_nonselected_nonexecuted_string_zero_scope`
    - `selected_status_counts_zero_raw_mixed_selected_nonselected_nonexecuted_zero_scope`
    - `selected_status_counts_zero_raw_mixed_selected_nonselected_nonexecuted_string_zero_scope`
  - Added non-selected-only retry/duration evidence (`retriedGateIds`, `gateRetryCountById`, `totalRetryBackoffSeconds`, `gateDurationSecondsById`, `executedDurationSeconds`, `retryBackoffSharePercent`) in integer and normalized numeric-string encodings to ensure selected non-executed explicit-list zero-denominator branches still suppress non-selected aggregates.
  - Assertions now additionally verify selected aggregate scoping for these explicit-list zero-denominator branches:
    - `Retried gates: none`, `Retried gate count: 0`, `Total retries: 0`, `Total retry backoff: 0s`
    - `Executed duration total: 0s`, `Executed duration average: n/a`, `Retry backoff share (executed duration): n/a`
    - non-selected retry/duration aggregates remain suppressed alongside prior explicit-list selected non-executed zero-denominator executed/rate assertions.
  - `scripts/README.md` selected aggregate precedence notes now explicitly mention explicit-list zero-denominator branch coverage for non-selected retry/duration suppression under selected non-executed scope.
  - Validation:
    - `./scripts/test-verify-gates-summary.sh` ✅
    - `make lint` ✅
    - `./scripts/verify-gates.sh --quick` ✅
...
- **Selected mixed explicit-list zero-raw scalar suppression parity (2026-02-14 PM)** Backfilled non-selected retry/duration evidence plus conflicting aggregate scalar bundles into `selected-status-counts-zero-raw-mixed-selected-nonselected-{scope,string-scope}` payloads and retained suppression assertions (`Retry rate: 0%`, retry/duration totals zeroed, share `n/a`, no leaked `90%`/`80%`/`4s`/`7`/`8s`/`6s`).
  **Why:** closes a false-positive gap where executed-denominator zero-raw selected explicit-list assertions existed without conflicting scalar evidence in payloads.

- **Selected mixed partition/map executed-denominator retry/duration scalar suppression parity (2026-02-14 PM)** Backfilled non-selected retry/duration evidence plus conflicting aggregate scalar bundles into mixed selected/non-selected partition-only and map-only scope/string-scope payloads (`partial-malformed` + `zero-raw`) and extended assertions to require selected-scope retry/duration aggregates remain scoped (`Retry rate: 0%`, retried totals/backoff `0`, duration total/average `0s`, retry-backoff share `n/a`) while suppressing leaked scalar values (`90%`, `80%`, `4s`, `7`, `8s`, `6s`).
  **Why:** closes executed-denominator selected-scope parity gaps beyond explicit-list branches so partition/map fallback paths now prove aggregate suppression against conflicting non-selected evidence.

- **Selected mixed zero-scope retry/duration scalar suppression parity (2026-02-14 PM)** Completed mixed selected/non-selected executed-denominator parity by backfilling conflicting non-selected retry/duration evidence + scalar bundles into remaining zero-scope payloads (explicit-list, partition-only, map-only; partial-malformed + zero-raw; integer + normalized numeric-string) and adding suppression assertions for all corresponding step summaries (`Retry rate: 0%`, retried totals/backoff `0`, executed duration total/average `0s`, retry-backoff share `n/a`, no leaked `90%`/`80%`/`4s`/`7`/`8s`/`6s`).
  **Why:** closes the final mixed selected/non-selected aggregate hardening gap so both scope and zero-scope selected executed-denominator branches enforce the same retry/duration suppression guarantees.
...
- **Selected no-evidence nonselected/map-only scalar suppression parity (2026-02-14 PM)** Backfilled conflicting retry/duration aggregate scalar bundles into selected no-evidence nonselected-evidence + nonselected-map-only payloads (`partial-malformed` + `zero-raw`; scope/string-scope/zero-scope/string-zero-scope), then hardened each leakage assertion to reject conflicting scalar outputs (`Retry rate 90%`, `Pass rate 80%`, `Total retry backoff 4s`, `Total retries 7`, `Executed duration total 8s`, `Executed duration average 6s`) alongside existing executed/count leakage guards.
  **Why:** closes a selected no-evidence hardening gap so scoped-out non-selected evidence cannot leak retry/duration/rate scalar aggregates into selected summaries across both explicit-list-backed and map-only fallback branches.

- **Selected run-state nonselected-evidence scalar suppression parity (2026-02-14 PM)** Enriched `selected-run-state-nonselected-evidence-scope` payload with conflicting non-selected retry/duration aggregate scalar bundles (`retriedGateCount`, `totalRetryCount`, `totalRetryBackoffSeconds`, `executedDurationSeconds`, `averageExecutedDurationSeconds`, `retryRatePercent`, `passRatePercent`, `retryBackoffSharePercent`) and extended assertions to require selected-scope fallback rendering (`Total retries: 0`, `Retried gates: none`, `Total retry backoff: 0s`, rate/duration fields `n/a`/`0s`) while explicitly rejecting leaked conflicting outputs (`90%`, `80%`, `4s`, `7`, `8s`, `6s`).
  **Why:** closes a run-state scoped-no-evidence gap so selected run-state summaries also enforce aggregate scalar suppression when all retry/duration evidence belongs to non-selected gates.

- **Selected run-state no-evidence scalar-only suppression parity (2026-02-14 PM)** Enriched `selected-run-state-no-evidence-scope` payload with conflicting scalar-only retry/duration/rate aggregates (`retriedGateCount`, `totalRetryCount`, `totalRetryBackoffSeconds`, `executedDurationSeconds`, `averageExecutedDurationSeconds`, `retryRatePercent`, `passRatePercent`, `retryBackoffSharePercent`) and extended assertions to require selected no-evidence fallback rendering (`Total retries: 0`, `Retried gates: none`, `Total retry backoff: 0s`, rate/duration fields `n/a`/`0s`) while explicitly rejecting leaked conflicting scalar outputs (`90%`, `80%`, `4s`, `7`, `8s`, `6s`).
  **Why:** closes the remaining selected run-state scalar-only no-evidence gap so scalar aggregate overrides cannot bypass selected no-evidence fallback behavior.

- **Selected run-state unknown/partial status scalar suppression parity (2026-02-14 PM)** Enriched `selected-run-state-unknown-status-scope` and `selected-run-state-partial-status-scope` payloads with conflicting non-selected retry/duration evidence plus scalar bundles (`retriedGateIds`, `gateRetryCountById`, `retriedGateCount`, `totalRetryCount`, `totalRetryBackoffSeconds`, `executedDurationSeconds`, `averageExecutedDurationSeconds`, `retryRatePercent`, `passRatePercent`, `retryBackoffSharePercent`) and extended assertions to require selected-scope aggregate projections (`unknown-status`: retries/backoff/duration `0` + rates `n/a`; `partial-status`: retries/backoff/duration `0` + retry rate `0%` + pass rate `100%`) while explicitly rejecting leaked conflicting scalar outputs (`90%`, `80%`, `4s`, `7`, `8s`, `6s`).
  **Why:** closes selected run-state unresolved/partial-status aggregate hardening gaps so conflicting scalar bundles cannot override selected-scope status-driven aggregate derivation.

- **Selected run-state not-run scalar suppression parity (2026-02-14 PM)** Enriched `selected-run-state-not-run-scope` payload with conflicting non-selected retry/duration evidence plus scalar bundles (`retriedGateIds`, `gateRetryCountById`, `retriedGateCount`, `totalRetryCount`, `totalRetryBackoffSeconds`, `executedDurationSeconds`, `averageExecutedDurationSeconds`, `retryRatePercent`, `passRatePercent`, `retryBackoffSharePercent`) and extended assertions to require selected non-executed aggregate fallback rendering (`Total retries: 0`, `Retried gates: none`, `Total retry backoff: 0s`, rate/duration fields `n/a`/`0s`) while explicitly rejecting leaked conflicting scalar outputs (`90%`, `80%`, `4s`, `7`, `8s`, `6s`).
  **Why:** closes the selected run-state non-executed aggregate hardening gap so conflicting scalar retry/duration bundles cannot leak through selected not-run scope.

- **Selected run-state blocked selected/nonselected scalar suppression parity (2026-02-14 PM)** Enriched `selected-run-state-not-run-blocked-selected-scope` and `selected-run-state-not-run-blocked-nonselected-scope` payloads with conflicting non-selected retry/duration evidence + scalar bundles (`retriedGateIds`, `gateRetryCountById`, `retriedGateCount`, `totalRetryCount`, `totalRetryBackoffSeconds`, `executedDurationSeconds`, `averageExecutedDurationSeconds`, `retryRatePercent`, `passRatePercent`, `retryBackoffSharePercent`) and extended assertions to require selected non-executed aggregate fallback rendering (`Total retries: 0`, `Retried gates: none`, `Total retry backoff: 0s`, rate/duration fields `n/a`/`0s`) while explicitly rejecting leaked conflicting scalar outputs (`90%`, `80%`, `4s`, `7`, `8s`, `6s`).
  **Why:** closes blocked-reason selected-scope aggregate hardening gaps so conflicting scalar retry/duration bundles cannot leak under either selected blocked-by fail-fast derivation or scoped-out non-selected blocked reason branches.

- **Selected run-state scalar-blocked/nonselected-blocked scalar suppression parity (2026-02-14 PM)** Enriched `selected-run-state-scalar-blocked-{only,whitespace,empty,continue,dry-run,dry-reason,continued-conflict}-scope` plus `selected-run-state-nonselected-blocked-scope` payloads with conflicting non-selected retry/duration evidence + scalar bundles (`retriedGateIds`, `gateRetryCountById`, `retriedGateCount`, `totalRetryCount`, `totalRetryBackoffSeconds`, `executedDurationSeconds`, `averageExecutedDurationSeconds`, `retryRatePercent`, `passRatePercent`, `retryBackoffSharePercent`) and extended assertions to require selected aggregate fallback rendering (`Total retries: 0`, `Retried gates: none`, `Total retry backoff: 0s`, rate/duration fields `n/a`/`0s`) while explicitly rejecting leaked conflicting scalar outputs (`90%`, `80%`, `4s`, `7`, `8s`, `6s`).
  **Why:** closes scalar blocked-by selected/no-evidence run-state aggregate hardening gaps so conflicting scalar retry/duration bundles cannot leak through either selected fail-fast scalar blocked paths or scoped-out non-selected scalar blocked paths.
